...
// import { productApi, userApi, categoryApi, order } from '../services/api';
    ...
    const loadStats = async () => {
        try {
            ...
            if (isAdmin()) {
                try {
                    const usersRes = await userApi.getAll({ page: 1, pageSize: 1 });
                    usersCount = usersRes.data.totalCount || usersRes.data?.items?.length || usersRes.data?.length || 0;
                } catch (e) {
                    console.log('Cannot fetch users count');
                }
            }
            ...
            setStats({
                products: productsRes.data?.totalCount || productsRes.data?.items?.length || productsRes.data?.length || 0,
                categories: categoriesRes.data?.totalCount || categoriesRes.data?.items?.length || categoriesRes.data?.length || 0,
                users: usersCount,
                orders: ordersRes.data?.totalCount || ordersRes.data?.items?.length || ordersRes.data?.length || 0,
            });
        } catch (error) {
            console.error('Failed to load stats:', error);
        } finally {
